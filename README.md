## <p align="center">Асылбек уулу Бакыт</p>

|Возраст|Телеграм|Почта|GitHub|Гражданство|
|:----:|:----:|:----:|:----:|:----:|
|19 лет|https://t.me/fred_nekrasov|frednekrasov49@gmail.com|https://github.com/FredNekrasov|Кыргызская Республика|

Среднее специальное образование
---
**Учебное заведение**: Федерального государственного бюджетного образовательного учреждение высшего образования "Московский государственный университет технологии и управления имени К. Г. Разумовского (Первый казачий университет)" Университетского колледжа информационных технологий  
**Специальность**: Информационные системы и программирование  
**Курс**: 4  
**Год поступления**: `2020` **Год выпуска**: `2024`

***Знание языков***
- Кыргызский – Native – Родной.
- Русский – C2 – В совершенстве
- Английский – B1 – Средний

|Качества|
|:----:|
Исполнительность
Саморазвитие
Ответственность
...

**Навыки**:  
+ Языки программирования: C#, Java, Kotlin.
+ Среды разработки: MS Visual Studio, Intellij IDEA, Android Studio.
+ Платформы: .NET Framework, .NET Core, ASP.NET Core, Android SDK.
+ Базы данных MS SQL Server, MySQL, SQLite.
+ Парадигмы, принципы и архитектуры: ООП, SOLID, KISS, DRY, Single Activity, REST, MVVM, MVI, Clean architecture.
+ Тестирование, умение разбираться в чужом коде, рефакторинг кода.

Дополнительная информация
|Место проживания|Семейное положение|
|:----:|:----:|
|г. Москва, метро Некрасовка|не женат|

[![Fred's GitHub stats](https://github-readme-stats.vercel.app/api?username=FredNekrasov&show_icons=true&theme=vue&border_radius=9)](https://github.com/anuraghazra/github-readme-stats)
[![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=FredNekrasov&theme=vue&border_radius=9&langs_count=9&layout=compact)](https://github.com/anuraghazra/github-readme-stats)
