...
## Обо мне
Привет! Я Android разработчик с опытом работы в разработке мобильных приложений на `Kotlin`. Мои проекты включают использование `Views`/`Jetpack Compose`, `Coroutines`, `Retrofit2`, `Room`, `Dagger-Hilt`/`Koin` для создания приложений. Я применяю принципы `SOLID` и `Clean architecture`, использую `Single Activity`, работаю с `REST API`, и использую архитектурные паттерны `MVVM`, `MVI` для создания удобного и масштабируемого кода.
